...
export class PermissionService {
  ...
  private static getRole(config?: {
    ...
    if (this.isArticleContext(context)) {
      if (context.articleAuthorId === session.nickname) return 'author'
    }

    if (this.isCommentContext(context)) {
      if (context.commentAuthorId === session.nickname) return 'commenter'
    }

    if (this.isProfileContext(context)) {
      if (context.profileOwnerId === session.nickname) return 'owner'
    }
